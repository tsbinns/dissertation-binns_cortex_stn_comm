%% Create interpolated MIC patterns and plot raw points (Figures 6B & 11)
...
%% MED OFF and ON averaged (Figure 6B)
...
for region_i = 1:length(regions)
    ...
    for fband_i = 1:length(fbands)
        ...
        fband_table = region_table(ci(fband, region_table.frequencies_band_labels), :);
        coords = [];
        patterns = [];
        ...
        for idx = 1:size(fband_table, 1)
            ...
            patterns(idx,1) = fband_table.connectivity_mic_topographies(idx);
        end
...
%% MED OFF and ON separately (Figure 11)
